...
import { COLORS, CARD_COLORS } from '../data/theme.js'
...
export default class Card3D {
  ...
  _createGlow() {
    ...
    const mat = new THREE.MeshBasicMaterial({ color: new THREE.Color(color), transparent: true, opacity: 0, side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false })
  ...
  _createCard() {
    ...
    const mat = new THREE.MeshBasicMaterial({ color: 0x0a0e1a, transparent: true, opacity: 0.92, side: THREE.DoubleSide })
    this.card = new THREE.Mesh(geo, mat)
    this.group.add(this.card)
    const innerMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(color), transparent: true, opacity: 0.04, side: THREE.DoubleSide })
    const inner = new THREE.Mesh(new THREE.PlaneGeometry(w - 0.08, h - 0.08), innerMat)
  ...
  _createEdges() {
    const color = CARD_COLORS[this.index] || COLORS.primary
    this.edgeMat = new THREE.LineBasicMaterial({ color: new THREE.Color(color), transparent: true, opacity: 0.25 })
    this.edges = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.PlaneGeometry(2.0, 1.4)), this.edgeMat)
  ...
  _createTextLabel() {
    ...
    this.label.scale.set(3.0, 0.9, 1)
    this.label.position.set(0, -0.3, 0.2)
  ...
  setHover(hovered) {
    ...
    this.group.scale.lerp(new THREE.Vector3(hovered ? 1.08 : 1, hovered ? 1.08 : 1, 1), 0.15)
  }
